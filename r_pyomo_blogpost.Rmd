...
My goal in this blog is to see how far I can get in terms of using Pyomo from R using the [reticulate](https://rstudio.github.io/reticulate/) package. The simplest option would be to develop the model in pyomo and call it from R using reticulate. While that is a simple option and will work, it still requires writing the pyomo model in python. I want to use reticulate to be able to write the pyomo model using R. This blog catalogs my learnings by trying to develop a few optimization models in pyomo from R.
...
For using pyomo package through reticulate, it is necessary to have python and pyomo already installed. Once python and pyomo is there we can import pyomo as follows:
...
I also ran into some signal handling errors. I added the following two commands to turn off signal handling (based on this [thread](https://github.com/PyUtilib/pyutilib/issues/31)). I don't know if it has other side effects.
...
$$ 
Since arithmetic operators in pyomo are overloaded to generate pyomo expressions, I wrote functions for [arithmetic operators] based on this [discussion thread]() and sourced them here.

```{r source-operators}
source("operators.R")
```

The pyomo model using python for this problem is in this [location](). As you see the R code, you will see that reticulate makes it easy to write code close to what it is in python (with certain modifications)
...
Given a set of markets with demands and plants that can supply the demands, the goal in the transportation problem is to determine minimum cost shipping scenario that meets market demands while satisfying supply capacity of plants. The python pyomo version of this example is in this [location](). This decision variables and parameters of the problem are:
...
Next, we create model object and define the parameters and decision variables
```{r}
...
Next, we will set the constraint on supply from plants being below their capacities. In pyomo, the constraints can be described using a function that describes the rule to construct the constraint. In python pyomo, the supply constraint will be list as follows:
...
The function `supply_rule` is a function that tells what the constraint expression is for a particular plant $i$. Then `M.supply_cons` line ensures that the constraint is generated for each of the plants. Python shines here since this is represented succintly using list comprehensions. I wasn't sure if using R function to represent the rule will work. But while I had to use loops to represent constraint expression, passing the R function to create the supply constraint just worked (I guess that's the magic of reticulate). Below is the supply constraint constructed in R.
...
* Single machine scheduling problem: This is taken from Jeffrey Kantor's pyomo cookbook gallery. The goal here is to schedule a set of jobs which have a certain release date, due date and duration on a machine to minimize total delays. This is a mixed-integer linear program and the pyomo modeling uses the disjunction construct to specify certain constraints. 
* Pyomo has a DAE (Differential-Algebraic Equation) module that make it easy to specifiy dynamic optimization and control problems. The optimal control example from pyomo repo is coded using R in this location.
* An example of parameter estimation for a reaction system is coded in this location. This example is from the book Chemical Reactor Design and Analysis by Rawlings and Eckerdt.

### Summary

Here I covered 2 examples to show how to develop a pyomo model from R using the reticulate package. While, it might still be easier to develop the pyomo model in python (since it was meant to be that way), I found that it is possible to develop pyomo models in R also fairly easily albeit with some modifications (some maybe less elegant compred to the python counterpart). I am summarizing some key learnings:
...
* Both pyomo and reticulate are awesome packages and many thanks to its creators.